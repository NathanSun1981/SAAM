...
def parse_args():
    ...
    parser.add_argument('--device', default='cuda', help='The device to run generation on.')
    ...
    parser.add_argument('--dataset', type=str, default='ade20k', choices=['ade20k', 'cityscapes', 'drone'], help='specify the set of class names')
    parser.add_argument('--seg_model', type=str, default='oneformer', choices=['oneformer', 'segformer', 'yolo'], help='specify the segmenta model')
...
def segmentor_inference(filename, output_path, img=None, save_img=False,
    ...
    if save_img:       
        ...
        imshow_det_bboxes(img,
                            ...
                            font_size=25,
                            show=False,
                            out_file=os.path.join(output_path, filename + '_' + model + '_semantic.png'))
        print('save predictions to ', os.path.join(output_path, filename + '_semantic.png'))
    mmcv.dump(anns, os.path.join(output_path, filename + '_' + model + '_semantic.json'))
...
def main(args): 
    ...
    if args.seg_model == 'oneformer':
        ...
        if args.dataset == 'ade20k':
            ...
            semantic_branch_model = OneFormerForUniversalSegmentation.from_pretrained(
                "shi-labs/oneformer_ade20k_swin_large")
        elif args.dataset == 'cityscapes' or args.dataset == 'drone':
            semantic_branch_processor = OneFormerProcessor.from_pretrained(
    ...
    for i, file_name in enumerate(local_filenames):
        t = time.time()
        print('Processing ', i, '/', len(local_filenames), ' ', file_name+ext)
